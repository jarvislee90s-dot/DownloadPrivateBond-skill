...
DownloadPrivateBond/
  ...
  output/                       # 生成的 Excel、WIND 结果 JSON 和日志
  Download/                     # 下载的 PDF
...
`company_name` 可选；当前模板只要求 `bond_short_name` 必填，公司名称由 WIND 公式自动查询。
...
output\信评需求私募债_YYYYMMDD_HHMMSS.json
```
...
- 将债券简称填入 A 列
- 刷新 WIND 公式
- 从 Excel 缓存值导出同名 JSON，保留未过滤、未排序的 WIND 原始结果
- 如果 Excel COM 报错但缓存值完整，继续写 JSON 和最终 Excel
- 如果 WIND 返回错误码（如 `-214682...`）或缓存值不完整，停止处理，避免固化坏值
- 删除发行方式为“公募”的行
- 按发行人名称升序排序

### 3. 下载募集说明书

默认自动读取 `output` 目录下最新的 `信评需求私募债_*.json`：
...
```powershell
python skills\download-private-bond\scripts\download_ratingdog_announcements.py --json "output\信评需求私募债_YYYYMMDD_HHMMSS.json"
```

旧参数 `--excel output\xxx.xlsx` 仍兼容，但脚本只会转为同名 `xxx.json`，不会读取 Excel 文件。读取 JSON 后会排除发行方式为“公募”的债券；第一轮处理时还会排除债券简称包含 `PPN` 的债券。
...
python -m py_compile skills\download-private-bond\scripts\prepare_bond_excel.py skills\download-private-bond\scripts\download_ratingdog_announcements.py skills\download-private-bond\scripts\cleanup_residual_files.py
```

## 注意事项

- 不要提交 `skills\download-private-bond\.env`。
- 不要提交批量生成的 Excel、WIND 结果 JSON、日志、PDF 下载结果。
- 如果 WIND 返回错误码或债券全称缺少年份，下载脚本会跳过并写入日志。
...
- **WIND 返回错误码或缓存值不完整**：停止生成最终结果，避免把错误值固化
- **债券全称无年份**：跳过，写入日志
- **搜索结果为空**：检查日期范围是否正确
